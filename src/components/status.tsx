...
export function Status({ status, isAbbreviated = false }: StatusProps) {
  ...
  return (
    <div
      className={`shadow-status flex h-5 w-fit gap-0.5 whitespace-nowrap rounded-full border border-white pl-0.5 pr-1.5 text-xs font-semibold uppercase ${styles}`}
      data-testid={`status-pill-${status.replace(/\s/g, "-")}`}
    >
      <Icon />
